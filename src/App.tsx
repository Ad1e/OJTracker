...
export default function App() {
  ...
  const [profile, setProfile] = useState<TraineeProfileData | null>(() => {
    try {
      const stored = JSON.parse(localStorage.getItem("practilog_profile") ?? "null");
      if (stored) {
        // Sync hardcoded values with the JSON if they are present
  ...
  // Late instances: days where hoursWorked < expected
  const lates = useMemo(() => {
    const getExpectedHours = (date: string) => {
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 pb-12 space-y-6">
        ...
        {activeTab === "dashboard" && (
          <>
            ...
            {/* ── Row 2: Compact inline progress bar ───────────────────── */}
            <section className="glass-card px-6 py-4 hover:!translate-y-0">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-xs text-slate-500 tabular-nums">
                    {stats.remaining > 0 ? `${stats.remaining.toFixed(1)} h remaining` : "✅ All done!"}
                  </span>
                  <span className="text-xl font-display font-bold text-accent tabular-nums drop-shadow-[0_0_8px_rgba(99,102,241,0.5)]">
              ...
              <div className="relative h-3 bg-slate-800/80 rounded-full overflow-hidden border border-slate-700/50">
                ...
                {[25, 50, 75].map((pct) => (
                  <div key={pct} className="absolute inset-y-0 w-px bg-slate-700/80" style={{ left: `${pct}%` }} />
                ))}
              </div>
            ...
            {/* ── Row 3a: Predictor (2/3) + Recent Activity (1/3) ──────────── */}
            <section>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                ...
                <div className="lg:col-span-2">
                  <PredictorCard logs={logs} target={TARGET} />
                </div>

                {/* Recent Activity — aligned to Predictor top */}
                <div className="lg:col-span-1">
                  <div className="glass-card overflow-hidden hover:!translate-y-0">
                    <div className="px-4 py-3 border-b border-slate-700/50 bg-slate-800/30 flex items-center justify-between">
                      ...
                      <span className="text-[10px] text-slate-500 bg-surface border border-slate-700/50 px-2 py-0.5 rounded-full tabular-nums">
                        {logs.filter((l) => !l.isHoliday).length} work days
                      </span>
                    </div>
                    ...
                    <div className="px-4 py-2.5 border-t border-slate-700/50 bg-slate-800/20">
                      <p className="text-[10px] text-slate-500 text-center">See full log below ↓</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {/* ── Row 3b: Weekly Breakdown (full width) ─────────────────────── */}
            <section>
              <WeeklyCard logs={logs} target={TARGET} />
            </section>
                ...
                <div className="glass-card overflow-hidden hover:!translate-y-0">
                  ...
                  <div className="flex items-center justify-between px-5 py-3 border-b border-slate-700/50 bg-slate-800/30">
                    <div className="flex items-center gap-2">
                      ...
                      <h2 className="text-sm font-display font-semibold text-white">Late Instances</h2>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-[10px] text-slate-500 tabular-nums">{lates.length} late day{lates.length !== 1 ? "s" : ""}</span>
                      <span className="text-xs font-bold text-red-400 tabular-nums bg-red-500/10 border border-red-500/20 px-2.5 py-0.5 rounded-full">
                        -{totalDeducted}h total
                      </span>
                    </div>
                  </div>
                          ...
                          <tr key={l.id} className="hover:bg-slate-800/40 transition-colors">
                            ...
                            <td className="px-5 py-3 text-right">
                              <span className="inline-flex items-center gap-1 text-red-400 font-bold tabular-nums">
                                <span className="text-red-500/60">-</span>{l.deductionMins} min
                              </span>
                            </td>
                          </tr>
                        ))}
                      ...
                      <tfoot>
                        <tr className="border-t border-slate-700/50 bg-slate-800/30">
                          ...
                          <td className="px-5 py-2.5 text-right">
                            <span className="text-red-400 font-bold tabular-nums">-{totalDeducted}h</span>
                          </td>
                        </tr>
                      </tfoot>
                    </table>
