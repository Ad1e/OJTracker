...
// Auth screen. Stays in components/ root as a layout-level component.
// Replaced "catch (err: any)" with typed narrowing.
// ─────────────────────────────────────────────────────────────────────────────
...
export function Auth({ onLogin }: { onLogin: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!supabase) {
      setTimeout(() => { localStorage.setItem("mock_session", "true"); onLogin(); }, 500);
      return;
    }
        ...
        <div className="glass-card py-8 px-4 shadow-2xl sm:rounded-2xl sm:px-10 border border-slate-800">
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              >
                {loading && <svg className="w-4 h-4 mr-2 animate-spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4" /></svg>}
                {isLogin ? "Sign in" : "Sign up"}
              </button>
            </div>
          </form>
          <div className="mt-8 pt-6 border-t border-slate-700/50 text-center">
            <button onClick={() => { setIsLogin(!isLogin); setError(null); }} className="text-slate-400 hover:text-white transition-colors text-sm font-medium">
