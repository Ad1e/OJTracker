...
// Renamed from profilecard.tsx. TraineeProfileData and HoursCalcResult
// imported from src/types instead of component siblings.
// ─────────────────────────────────────────────────────────────────────────────
import type { TraineeProfileData, HoursCalcResult } from "../../../types";
...
function ProfileAvatar({ src, name, size = "md" }: { src: string | null; name: string; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div className={`${dim} rounded-full shrink-0 ring-1 ring-slate-600 overflow-hidden flex items-center justify-center bg-slate-800`}>
      {src ? <img src={src} alt={name} className="w-full h-full object-cover" /> : <span className="font-display font-bold text-slate-300 select-none">{getInitials(name) || "PL"}</span>}
    </div>
  );
}
...
export function ProfileStrip({ profile, onEdit }: ProfileCardProps) {
  return (
    <div className="flex items-center gap-3">
      ...
      {onEdit && (
        <button onClick={onEdit} title="Edit profile" className="p-1.5 rounded-lg text-slate-500 hover:text-accent hover:bg-slate-800 transition-all border border-transparent hover:border-slate-700" aria-label="Edit profile">
  ...
  return (
    <div className="glass-card p-5 relative overflow-hidden group">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-accent/10 text-indigo-300 ring-1 ring-accent/20">
              <span className="w-1 h-1 rounded-full bg-accent" />{profile.department}
            </span>
          </div>
        </div>
        {onEdit && (
          <button onClick={onEdit} className="p-2 rounded-xl text-slate-400 hover:text-white hover:bg-slate-700/50 transition-colors border border-transparent hover:border-slate-700" aria-label="Edit profile">
            <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
          </button>
        )}
      </div>
      ...
      <div className="space-y-2.5">
        ...
        <DetailRow label="Start Date" value={profile.startDate ? new Date(profile.startDate + "T00:00:00").toLocaleDateString("en-PH", { year: "numeric", month: "long", day: "numeric" }) : "Not set"} />
        {stats && (
          <>
            ...
            <div className="flex items-start gap-3 p-3 rounded-lg bg-indigo-500/10 border border-indigo-500/20">
              <svg className="w-4 h-4 mt-0.5 shrink-0 text-indigo-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M13 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V9z" /><polyline points="13 2 13 9 20 9" /></svg>
              <div className="min-w-0">
                <p className="text-[11px] text-indigo-300 uppercase tracking-wider font-semibold">Est. Completion</p>
                <p className="text-xs text-indigo-200 font-semibold mt-1 break-words">{stats.estimatedCompletionDate || "All hours completed! 🎉"}</p>
                {stats.remaining > 0 && <p className="text-[10px] text-indigo-300/70 mt-1">{stats.remaining.toFixed(1)}h remaining · {stats.avgHoursPerDay.toFixed(1)}h/day avg</p>}
              </div>
            </div>
          </>
        )}
