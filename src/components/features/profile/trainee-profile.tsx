// ─── src/components/features/profile/trainee-profile.tsx ─────────────────────
// Renamed from traineeprofile.tsx. TraineeProfileData imported from src/types.
// The inline export of TraineeProfileData is removed — use src/types/index.ts.
// ─────────────────────────────────────────────────────────────────────────────
import React, { useState, useRef, useCallback, useEffect } from "react";
import type { TraineeProfileData } from "../../../types";
...
function AvatarUpload({ value, name, onChange }: {
  value: string | null; name: string; onChange: (dataUrl: string | null) => void;
}) {
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <div
        className={`relative w-24 h-24 rounded-full cursor-pointer group transition-all duration-200 ${dragging ? "ring-2 ring-accent ring-offset-2 ring-offset-slate-900 scale-105" : "ring-1 ring-slate-700 hover:ring-accent/50"}`}
      ...
      >
        {value
          ...
          : <div className="w-full h-full rounded-full bg-slate-800 flex items-center justify-center"><span className="font-display text-2xl font-bold text-slate-400 select-none">{initials}</span></div>
        }
      ...
      {value && <button type="button" onClick={() => onChange(null)} className="text-[11px] text-slate-500 hover:text-red-400 transition-colors font-medium">Remove photo</button>}
    </div>
  );
}
  ...
  return (
    <div className="flex items-center gap-2">
      {Array.from({ length: total }).map((_, i) => (
        <div key={i} className={`rounded-full transition-all duration-300 ${i === current ? "w-6 h-1.5 bg-accent shadow-[0_0_8px_rgba(99,102,241,0.6)]" : i < current ? "w-1.5 h-1.5 bg-indigo-700" : "w-1.5 h-1.5 bg-slate-700"}`} />
      ))}
    </div>
  );
}
...
export function TraineeProfile({ onSave, initial, onCancel, canCancel = true }: TraineeProfileProps) {
  ...
  const handleNext = () => {
    const e = validateStep0(); setSubmitted(true);
    if (Object.keys(e).length > 0) { setErrors(e); return; }
    setErrors({}); setStep(1);
  };
  const handleSave = () => {
    const e = validateStep1(); setSubmitted(true);
    if (Object.keys(e).length > 0) { setErrors(e); return; }
    onSave(form);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
      <div className="relative w-full max-w-lg glass-card overflow-hidden bg-surface animate-slide-up">
        <div className="relative px-6 pt-6 pb-5 border-b border-slate-700/50 bg-slate-800/30 overflow-hidden">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="text-lg font-display font-bold text-white tracking-tight">{step === 0 ? "Your Profile" : "Internship Details"}</h2>
              <p className="text-xs text-slate-400 mt-0.5">{step === 0 ? "Tell us who you are." : "Configure your internship parameters."}</p>
            </div>
        ...
        {step === 0 && (
          <div className="px-6 py-5 flex flex-col gap-5">
            ...
            <Field label="Full Name" required error={submitted ? errors.name : undefined}>
              <input type="text" value={form.name} placeholder={journalData.trainee} onChange={(e) => set("name", e.target.value)} className={submitted && errors.name ? inputErrorClass : inputClass} autoFocus />
            </Field>
            <Field label="Department / Section" required error={submitted ? errors.department : undefined}>
              <input type="text" value={form.department} placeholder={journalData.course} onChange={(e) => set("department", e.target.value)} className={submitted && errors.department ? inputErrorClass : inputClass} />
            </Field>
            <Field label="School / University">
              <input type="text" value={form.school} placeholder="Batangas State University" onChange={(e) => set("school", e.target.value)} className={inputClass} />
            </Field>
          </div>
        )}
          ...
          <div className="px-6 py-5 flex flex-col gap-5">
            <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-800/60 border border-slate-700/50">
              <div className="w-9 h-9 rounded-full shrink-0 overflow-hidden ring-1 ring-slate-600 flex items-center justify-center bg-slate-700">
                {form.avatarDataUrl ? <img src={form.avatarDataUrl} alt="" className="w-full h-full object-cover" /> : <span className="text-xs font-bold text-slate-300">{getInitials(form.name) || "PL"}</span>}
              </div>
            ...
            <Field label="Training Supervisor" required error={submitted ? errors.supervisor : undefined}>
              <input type="text" value={form.supervisor} placeholder={journalData.supervisor} onChange={(e) => set("supervisor", e.target.value)} className={submitted && errors.supervisor ? inputErrorClass : inputClass} autoFocus />
            </Field>
            <div className="grid grid-cols-2 gap-4">
              <Field label="Required Hours" required error={submitted ? errors.totalRequiredHours : undefined}>
                <input type="number" value={form.totalRequiredHours} min={1} max={9999} onChange={(e) => set("totalRequiredHours", parseInt(e.target.value, 10) || 500)} className={submitted && errors.totalRequiredHours ? inputErrorClass : inputClass} />
              </Field>
              <Field label="OJT Start Date">
                <input type="date" value={form.startDate} onChange={(e) => set("startDate", e.target.value)} className={inputClass} style={{ colorScheme: "dark" }} />
              </Field>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center justify-between px-6 pb-6 pt-2">
          <div className="flex items-center gap-2">
            {canCancel && onCancel && (
              <button type="button" onClick={onCancel} className="px-4 py-2.5 text-sm font-semibold text-slate-400 hover:text-slate-200 bg-transparent hover:bg-slate-800 rounded-xl transition-all border border-transparent hover:border-slate-700">Cancel</button>
            )}
            {step > 0 && (
              <button type="button" onClick={() => setStep(0)} className="px-4 py-2.5 text-sm font-semibold text-slate-400 hover:text-slate-200 bg-transparent hover:bg-slate-800 rounded-xl transition-all border border-transparent hover:border-slate-700">Back</button>
            )}
          </div>
          {step < TOTAL_STEPS - 1
            ? <button type="button" onClick={handleNext} className="px-6 py-2.5 text-sm font-semibold text-white bg-accent rounded-xl hover:bg-indigo-400 active:scale-95 transition-all shadow-[0_0_15px_rgba(99,102,241,0.3)]">Continue</button>
            : <button type="button" onClick={handleSave} className="px-6 py-2.5 text-sm font-semibold text-white bg-accent rounded-xl hover:bg-indigo-400 active:scale-95 transition-all shadow-[0_0_15px_rgba(99,102,241,0.3)]">Save Profile</button>
          }
        </div>
      </div>
    </div>
