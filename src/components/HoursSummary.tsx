...
export function HoursSummary({ weeks, targetHours = 500 }: HoursSummaryProps) {
  ...
  const thClass =
    "px-5 py-3 text-left text-xs font-bold uppercase tracking-widest text-slate-400";
  const tdClass =
    "px-5 py-3 text-sm text-slate-300 border-b border-slate-700/40";
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3 px-6 py-5 border-b border-slate-700/50 bg-slate-800/30">
          ...
          <div>
            <h3 className="text-base font-display font-bold text-white">
              Hours Summary (Weeks 1–{weeks.length})
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Weekly breakdown · {totalDays} days total
            </p>
          </div>
        </div>
          ...
          <table className="w-full min-w-[420px]">
              ...
              {summary.map((row, idx) => (
                <tr
                  ...
                  className={`transition-colors hover:bg-slate-800/50 ${
                    idx % 2 === 0 ? "bg-surface" : "bg-slate-800/20"
                  }`}
                >
                  {/* Week label */}
                  <td className={`${tdClass} font-semibold text-slate-200`}>
                    <span className="inline-flex items-center gap-2">
                      <span className="w-5 h-5 rounded-full bg-accent/10 border border-accent/20 text-accent text-[10px] font-bold flex items-center justify-center tabular-nums">
                        {row.week}
                      </span>
                      Week {row.week}
                    </span>
                    <p className="text-[11px] text-slate-500 font-normal mt-0.5 ml-7">
                      {row.period}
                    </p>
                  </td>
                  ...
                  <td className={`${tdClass} text-center tabular-nums`}>
                    <span className="inline-flex items-center justify-center w-8 h-6 rounded-md bg-slate-700/50 text-slate-300 text-xs font-semibold">
                      {row.days}
                    </span>
                  </td>
            ...
            <tfoot>
              <tr className="bg-slate-800/70 border-t-2 border-slate-600/60 shadow-[0_-4px_10px_rgba(0,0,0,0.3)]">
                <td className="px-5 py-4 text-sm font-display font-bold text-white uppercase tracking-wider">
                  TOTAL
                </td>
                ...
                <td className="px-5 py-4 text-right tabular-nums">
                  <span className="text-lg font-display font-bold text-accent drop-shadow-[0_0_8px_rgba(99,102,241,0.5)]">
      ...
      <div className="glass-card overflow-hidden hover:!translate-y-0">
        ...
        <div className="flex items-center gap-3 px-6 py-5 border-b border-slate-700/50 bg-slate-800/30">
          <span className="text-xl">⏳</span>
          <h3 className="text-base font-display font-bold text-white">
            Remaining to {targetHours} Hours
          </h3>
        </div>
        ...
        <div className="divide-y divide-slate-700/40">
          ...
          <div className="flex items-center justify-between px-6 py-4">
            ...
            <span
              className={`text-sm font-bold tabular-nums ${
                remaining === 0 ? "text-emerald-300" : "text-amber-300"
              }`}
            >
          ...
          <div className="flex items-center justify-between px-6 py-4">
            <span className="text-sm text-slate-400">
              Days needed{" "}
              <span className="text-slate-600">(10 hrs/day)</span>
            </span>
            <span
              className={`text-sm font-bold tabular-nums ${
                daysNeeded === 0 ? "text-emerald-300" : "text-indigo-300"
              }`}
            >
