// ─── src/pages/InternDashboardPage.tsx ───────────────────────────────────────
// Intern-only dashboard. Data is scoped to the authenticated intern's internId.
// Route: /dashboard  (InternRoute guard enforces intern-only access)
// ─────────────────────────────────────────────────────────────────────────────
...
export default function InternDashboardPage() {
  ...
  // ── Data — scoped to this intern's auth.users.id ───────────────────────
  const { entries: logs, loading, error, addEntry, updateEntry, deleteEntry } = useEntries(user?.id ?? null);
    ...
    <div className="min-h-screen bg-background text-slate-300 font-sans selection:bg-accent/30">
      ...
      <header className="glass-card sticky top-0 z-30 border-b border-slate-800 rounded-none rounded-b-2xl mb-8 mx-4 sm:mx-6 mt-2 max-w-6xl xl:mx-auto py-3">
        <div className="px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          ...
          <button onClick={signOut} title="Sign out" className="p-2 rounded-xl text-slate-400 hover:bg-slate-800 hover:text-white transition-colors">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 pb-12 space-y-6">
        ...
        <div className="flex gap-1.5 p-1 bg-slate-800/60 rounded-xl border border-slate-700/50 w-fit">
          {([ { id: "dashboard", label: "Dashboard", icon: "📈" }, { id: "summary", label: "Hours Summary", icon: "📊" } ] as const).map((tab) => (
      ...
      <footer className="text-center py-10 text-xs text-slate-600 font-medium border-t border-slate-800/50 mt-10">
        PractiLog · {new Date().getFullYear()} · Built with React + TypeScript + Supabase
      </footer>
